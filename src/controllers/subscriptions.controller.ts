...
async function confirmPlanChangeInMainService(
    ...
    paymentId: string,
    nextBillingDate: Date | null, // NEW: Pass nextBillingDate
    authToken: string // This authToken is actually paymentServiceApiKey, re-purposed from previous usage
): Promise<void> {
    ...
    try {
        await axios.put(
            `${config.mainServiceApiBaseUrl}/websites/${websiteId}/confirm-plan-change`,
            {
                ...
                paymentId: paymentId,
                nextBillingDate: nextBillingDate ? nextBillingDate.toISOString() : null, // NEW: Send nextBillingDate
            },
            {
                headers: {
...
// Helper function to inform main service about subscription cancellation
async function informMainServiceOfCancellation(
    websiteId: string,
    nextBillingDate: Date | null, // NEW: Pass nextBillingDate (null for cancellation)
    authToken: string // This authToken is actually paymentServiceApiKey
): Promise<void> {
    console.log(`[informMainServiceOfCancellation] Attempting to inform main service about subscription cancellation for website ${websiteId}.`);
    try {
        await axios.put(
            `${config.mainServiceApiBaseUrl}/websites/${websiteId}/cancel-subscription-confirmed`,
            {
                nextBillingDate: nextBillingDate ? nextBillingDate.toISOString() : null, // NEW: Send nextBillingDate
            },
            {
        ...
        console.log(`[informMainServiceOfCancellation] Successfully informed main service about subscription cancellation for website ${websiteId}.`);
    } catch (error: any) {
        console.error(`[informMainServiceOfCancellation] Failed to inform main service about subscription cancellation for website ${websiteId}:`, error.message);
        if (axios.isAxiosError(error) && error.response) {
            console.error('[informMainServiceOfCancellation] Main service error response:', error.response.data);
        }
        throw new Error(`Failed to inform main service about cancellation: ${error.message}`);
    }
}

// Helper function to inform main service about failed billing attempt
async function informMainServiceOfBillingFailure(
    websiteId: string,
    nextBillingDate: Date | null, // NEW: Pass nextBillingDate (next retry date)
    authToken: string // This authToken is actually paymentServiceApiKey
): Promise<void> {
    console.log(`[informMainServiceOfBillingFailure] Attempting to inform main service about billing failure for website ${websiteId}.`);
    try {
        await axios.put(
            `${config.mainServiceApiBaseUrl}/websites/${websiteId}/billing-failed`,
            {
                nextBillingDate: nextBillingDate ? nextBillingDate.toISOString() : null, // NEW: Send nextBillingDate
            },
            {
        ...
        console.log(`[informMainServiceOfBillingFailure] Successfully informed main service about billing failure for website ${websiteId}.`);
    } catch (error: any) {
        console.error(`[informMainServiceOfBillingFailure] Failed to inform main service about billing failure for website ${websiteId}:`, error.message);
        if (axios.isAxiosError(error) && error.response) {
            console.error('[informMainServiceOfBillingFailure] Main service error response:', error.response.data);
        }
        throw new Error(`Failed to inform main service about billing failure: ${error.message}`);
    }
}
    ...
    try {
        ...
        if (oldStripeSubscriptionId) {
            ...
            try {
                ...
                await stripe.subscriptions.del(oldStripeSubscriptionId, { invoice_now: true, prorate: true }); // prorate: true is important for upgrades/downgrades
...
// --- NEW ENDPOINT: Cancel an existing subscription ---
export async function cancelSubscription(request: FastifyRequest, reply: FastifyReply) {
    ...
    const { userId, websiteId } = request.body as { userId: string; websiteId: string }; // Required for verification
    const authToken = request.headers['x-auth-token'] as string;
    const mainServiceApiKey = request.headers['x-main-service-api-key'] as string; // Expecting this from main service

    console.log(`[cancelSubscription] Initiating cancellation for Stripe Subscription ID: ${stripeSubscriptionId}, userId: ${userId}, websiteId: ${websiteId}`);

    if (!stripeSubscriptionId || !userId || !websiteId || !authToken) {
        console.warn(`[cancelSubscription] Missing required parameters. stripeSubscriptionId: ${stripeSubscriptionId}, userId: ${userId}, websiteId: ${websiteId}, authToken present: ${!!authToken}`);
        return reply.status(400).send({ error: "Missing stripeSubscriptionId, userId, websiteId, or X-Auth-Token header." });
    }

    if (!mainServiceApiKey || mainServiceApiKey !== config.paymentServiceApiKey) {
        console.warn(`[cancelSubscription] Unauthorized attempt: Invalid Main Service API Key provided.`);
        return reply.status(401).send({ error: "Unauthorized: Invalid Main Service API Key." });
    }

    try {
        // 1. Retrieve the subscription to verify ownership and metadata
        console.log(`[cancelSubscription] Retrieving Stripe subscription ${stripeSubscriptionId} for verification.`);
        const subscription = await stripe.subscriptions.retrieve(stripeSubscriptionId);

        // Verify that the subscription belongs to the provided user and website
        if (subscription.metadata?.appUserId !== userId || subscription.metadata?.websiteId !== websiteId) {
            console.warn(`[cancelSubscription] Mismatch: Provided userId/websiteId (${userId}/${websiteId}) does not match subscription metadata (${subscription.metadata?.appUserId}/${subscription.metadata?.websiteId}).`);
            return reply.status(403).send({ error: "Unauthorized: Subscription does not belong to the provided user/website." });
        }

        // 2. Cancel the subscription immediately.
        // `at_period_end: false` means it cancels immediately, `true` means at the end of the current billing period.
        // For a full cancellation, usually `at_period_end: false` is desired.
        console.log(`[cancelSubscription] Calling Stripe to cancel subscription ${stripeSubscriptionId} immediately.`);
        const cancelledSubscription = await stripe.subscriptions.cancel(stripeSubscriptionId);
        console.log(`[cancelSubscription] Stripe Subscription ${stripeSubscriptionId} cancelled. New status: ${cancelledSubscription.status}`);

        // 3. Update the payment record status to CANCELED in your database
        console.log(`[cancelSubscription] Updating payment record for Stripe Subscription ID: ${stripeSubscriptionId} to CANCELED.`);
        const updatedPayment = await updatePaymentStatus(
            (await findPaymentByStripeSubscription(stripeSubscriptionId))?.id!, // Find the payment record associated
            PaymentStatus.CANCELED
        );
        console.log(`[cancelSubscription] Payment record for ${stripeSubscriptionId} updated to CANCELED. Payment ID: ${updatedPayment?.id || 'N/A'}`);

        // 4. Inform the Main Service about the cancellation
        console.log(`[cancelSubscription] Informing main service about cancellation for website ${websiteId}.`);
        // Pass null for nextBillingDate on cancellation
        await informMainServiceOfCancellation(websiteId, null, mainServiceApiKey); // Pass null for nextBillingDate

        return reply.status(200).send({ message: "Subscription cancelled successfully.", status: cancelledSubscription.status });

    } catch (error: any) {
        console.error(`[cancelSubscription] Error canceling subscription ${stripeSubscriptionId}:`, error);
        if (error.code === 'resource_missing') {
            return reply.status(404).send({ error: "Subscription not found.", details: error.message });
        }
        return reply.status(500).send({ error: "Failed to cancel subscription.", details: error.message });
    }
}
...
export async function handleWebhook(request: FastifyRequest, reply: FastifyReply) {
    ...
    switch (event.type) {
        ...
        case "customer.subscription.deleted":
            ...
            if (appUserId && websiteId) { // Ensure websiteId is available to call informMainServiceOfCancellation
                const existingPayment = await findPaymentByStripeSubscription(stripeSubscriptionId!); // Use ! as we know it's present from the if-block
                if (existingPayment) {
                ...
                // Inform Main Service about cancellation if not already done by direct API call
                // Pass null for nextBillingDate on cancellation
                await informMainServiceOfCancellation(websiteId, null, config.paymentServiceApiKey);
                console.log(`[handleWebhook] Successfully informed main service about subscription cancellation for website ${websiteId}.`);
            } else {
                console.warn(`[handleWebhook] Skipping customer.subscription.deleted processing: Missing appUserId or websiteId.`);
            }
            break;
            ...
            if (subscriptionIdFromInvoice && appUserId && websiteId) { // Ensure websiteId is available before calling confirmPlanChangeInMainService
                ...
                if (!websiteIdFromSubscription) {
                    try {
                        ...
                        websiteIdFromSubscription = subscription.metadata?.websiteId;
                ...
                const nextBillingDate = new Date(invoicePaymentSucceeded.period_end * 1000); // Stripe period_end is in seconds
                // Add one month for the next billing cycle
                    ...
                    if (paymentType === 'plan_change' || paymentType === 'initial_subscription') {
                        try {
                            ...
                            await confirmPlanChangeInMainService(
                                ...
                                paymentRecordId,
                                nextBillingDate, // NEW: Pass nextBillingDate
                                '' // Pass empty string as authToken, relies on x-payment-service-api-key
                            );
        ...
        case "invoice.payment_failed":
            ...
            if (failedSubscriptionId && appUserId && websiteId) { // Ensure websiteId is available
                ...
                // Determine next retry date (Stripe typically retries in 3, 5, 7 days etc.)
                // This is a simplified example. Stripe's dunning schedule is complex.
                // You might get this from invoice.next_payment_attempt or subscription.current_period_end
                const nextRetryDate = new Date();
                nextRetryDate.setDate(nextRetryDate.getDate() + 3); // Example: retry in 3 days

                // NEW: Inform main service that billing was unsuccessful
                try {
                    await informMainServiceOfBillingFailure(websiteId, nextRetryDate, config.paymentServiceApiKey); // Pass nextRetryDate
                    console.log(`[handleWebhook] Successfully informed main service about billing failure for website ${websiteId}.`);
                } catch (billingFailureError) {
                    console.error(`[handleWebhook] Failed to inform main service about billing failure from webhook for website ${websiteId}:`, billingFailureError);
                }
            } else {
                console.warn(`[handleWebhook] Skipping invoice.payment_failed processing: Missing subscriptionIdFromInvoice, appUserId, or websiteId.`);
            }
            break;
